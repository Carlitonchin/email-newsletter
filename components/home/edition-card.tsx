  ...
  return (
    ...
    >
      <Card className="transition-all duration-300 group-hover/edition:-translate-y-0.5 group-hover/edition:shadow-xl group-hover/edition:shadow-foreground/5 group-hover/edition:ring-foreground/20">
        <CardHeader className="flex flex-row items-center gap-3.5 sm:gap-4">
          <div
            className={cn(
              'grid shrink-0 place-items-center rounded-xl bg-muted ring-1 ring-foreground/5',
              featured ? 'size-13 sm:size-16' : 'size-12 sm:size-14',
            )}
          >
            <div className="text-center leading-none">
              <div className={cn('font-mono font-semibold tabular-nums', featured ? 'text-xl sm:text-2xl' : 'text-lg sm:text-xl')}>
                {formatDay(day.date)}
              </div>
              <div className="text-eyebrow mt-1.5 text-muted-foreground">{formatMonthShort(day.date)}</div>
            </div>
          </div>

          <div className="min-w-0 flex-1">
            {featured && <span className="text-eyebrow text-muted-foreground">Latest edition</span>}
            <CardTitle
              className={cn(
                'capitalize text-balance',
                featured && 'mt-1 line-clamp-2 text-base sm:line-clamp-none sm:text-lg',
              )}
            >
          ...
          <ArrowUpRightIcon className="size-5 shrink-0 self-center text-muted-foreground transition-transform duration-300 group-hover/edition:-translate-y-0.5 group-hover/edition:translate-x-0.5" />
        </CardHeader>
            ...
            <ul className="flex flex-col gap-2.5">
              {day.articles.map((article) => (
                <li key={article.slug} className="flex items-center gap-2.5 text-sm">
                  ...
                  <span className="min-w-0 truncate text-muted-foreground transition-colors group-hover/edition:text-foreground">
                    {article.title}
                  </span>
                </li>
              ))}
